...
export function generateTradeReceiptPdf(transaction: TransactionRecord): Uint8Array {
  ...
  const streamCommands: string[] = [
    ...
    "0.09 0.42 0.27 rg",
    "55 304 Td",
    "(DIGITAL VERIFICATION & AUDIT SEAL) Tj",
    "ET",
    "BT",
    "/F1 9 Tf",
    "0.3 0.35 0.4 rg",
    "55 288 Td",
    `(${escapePdf(transaction.razorpayPaymentId ? `Razorpay Payment ID: ${transaction.razorpayPaymentId}` : `Bank Reference: ${transaction.paymentReference || "SETU-NEFT-2026-9F4A28B1"}`)}) Tj`,
    "ET",
    "BT",
    "/F1 8.5 Tf",
    "0.35 0.4 0.45 rg",
    "55 272 Td",
    `(${escapePdf(transaction.razorpayOrderId ? `Razorpay Order ID: ${transaction.razorpayOrderId}` : "Digital Verification Ref: TXN-SETU-2026-SIH26132-VERIFIED")}) Tj`,
    "ET",
    "BT",
    "/F1 8.5 Tf",
    "0.4 0.45 0.5 rg",
    "55 256 Td",
    `(${escapePdf(`Payment Status: ${transaction.paymentStatus || "Verified & Settled"} · Auth Date: ${transaction.paymentDate || transaction.timestamp || "Official Digital Record"}`)}) Tj`,
    "ET",
    "BT",
    "/F1 8 Tf",
    "0.5 0.55 0.6 rg",
    "55 240 Td",
    "(Authenticated through KissanSetuAI Smart India Hackathon 2026 Engine · Direct Settlement Architecture) Tj",
    "ET",
    "BT",
    "/F1 7.5 Tf",
    "0.5 0.55 0.6 rg",
    "55 226 Td",
    "(This is a computer generated digital contract receipt and does not require physical signature.) Tj",
    "ET",
    "Q",
